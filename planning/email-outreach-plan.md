...
**Goal:** reactivate the 250K idle NovaGate emails into Arc users. Day-one distribution channel per deck p2.
...
1. **NovaGate prod DB** — original signup table.
2. **Nova CRM** (Customer.io / Iterable / Braze, whatever the company standard is — confirm).
3. An exported CSV from migration prep.

**Action D0:**
- [REDACTED] to confirm canonical source + access path with Nova's growth/CRM owner.
...
- GDPR re-consent for EU users if NovaGate's original opt-in didn't cover Arc (likely needs review).
- Suppression list scrubbed against Nova's master unsubscribe list (don't email someone who has opted out of Nova marketing — even from a different product).
...
| Tool | Why | Why not |
|---|---|---|
| **Customer.io** *(recommended)* | Behavior-driven sequencing; works well for "did open #1 + did fund wallet" branching that's central to this plan; webhook integration with Arc backend for trigger-on-action emails | Slower setup if Nova doesn't already use it |
| **Iterable** | If Nova already runs on it, free win — reuse warmed domain, existing suppression list | Heavyweight if standing up fresh |
...
**Recommendation:** *use whatever ESP Nova already runs on* (likely Iterable or Customer.io). Two reasons:

1. **Domain warmth.** Arc emails should send from `*.nova.com` or a sibling pre-warmed sending domain. Cold-warming `arc.com` takes 2–4 weeks and crushes inbox placement on a 250K blast.
2. **Suppression hygiene.** Reusing Nova's suppression list is non-negotiable — sending Arc to a previously-unsubscribed Nova customer is a compliance incident.

**Action D0–D3:** lock the tool with Growth ops. Stand up a separate Arc subdomain (`mail.arc.nova.com` or similar) under the existing warm IPs to avoid contaminating the main brand's sender reputation.

---

## 3. Segmentation

Three primary segments on the 250K, by *behavior in NovaGate*, not by demographics:

| Segment | Definition | Approx. size (assumed) | Hypothesis | First-send subject angle |
|---|---|---|---|---|
| **A — Funded** | Wallet funded ≥ 1x | ~15–25% | "Showed intent, didn't stick" — pull back with a concrete product upgrade | "Your NovaGate wallet just got smarter" |
...
- **From:** Real human name (e.g. "[REDACTED] at Arc" or whoever the brand voice is) — not a no-reply.
- **Subject A:** "We rebuilt NovaGate. It's called Arc."
- **Subject B (holdout):** "Your NovaGate account is now Arc — here's what changed"
- **Pre-header:** "Curated narrative baskets, agent-managed, Nova checkout."
- **Body skeleton:**
  > [Name] —
  >
  > Last year you signed up for NovaGate. We've been heads-down. Today we're shipping the version we wanted to build originally: **Arc**.
  >
  > Tap a theme. The agent builds the basket. Nova checks you out. Done.
  >
  ...
  > 3. Native Nova checkout — Apple Pay, Google Pay, card. No crypto wrangling.
  >
  > Your account moved over. **[See your Arc dashboard →]**
  >
  > — [Name]
  >
  > *PS: You can ignore everything from NovaGate. The handle, the look, the link — all replaced.*
...
- In-app brand swap complete (no NovaGate references anywhere a clicked-through user could land).
- Customer support trained on Arc-specific Qs (recovery flows, "what happened to my NovaGate account").
...
| `arc.com` (or domain) live with login flow that recognizes NovaGate accounts | Eng | D14 |
...
> Subject: We rebuilt NovaGate. It's called Arc.
> Pre-header: Curated baskets. Agent-managed. Nova checkout.
>
> Hey [first_name],
>
> A year ago you signed up for NovaGate. We took the feedback, the data, and a hard look — and rebuilt it.
>
> Today it's Arc. The pitch in one sentence: tap a theme, the agent builds the basket, Nova checks you out.
>
> What's different:
...
> - **Nova checkout**: Apple Pay, card. No crypto headache.
>
...
>
> [REDACTED]
> PS: This used to be "NovaGate." If you signed up under that name, same account — just a better product.
...
>
> Pick "Made in America." The agent assembles 8 xStocks weighted to today's macro. Nova does the checkout — Apple Pay, card, whatever.
>
